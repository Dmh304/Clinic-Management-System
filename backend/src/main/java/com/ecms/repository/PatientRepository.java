...
public interface PatientRepository extends JpaRepository<Patient, Long> {
    ...
    // Kiểm tra CCCD đã tồn tại trong DB chưa (CCCD là định danh chính, không cho trùng)
    boolean existsByCccd(String cccd);

    // Tìm bệnh nhân theo CCCD
    Optional<Patient> findByCccd(String cccd);

    // Tìm bệnh nhân trùng tên + ngày sinh (dùng để phát hiện trùng hồ sơ trẻ em không có CCCD)
    List<Patient> findByFullNameIgnoreCaseAndDateOfBirth(String fullName, LocalDate dateOfBirth);

    // Tìm kiếm bệnh nhân theo tên (không phân biệt hoa/thường), số điện thoại hoặc CCCD
