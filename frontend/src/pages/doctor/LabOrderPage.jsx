...
/* ------------------------------------------------------------------ */
/*  Status / Priority config — khớp với enum backend                  */
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
/*  Sub-component: 1 hàng so sánh kết quả 2 mắt                      */
/* ------------------------------------------------------------------ */
function EyeResultRow({ label, valueR, valueL, unit = '' }) {
  ...
  return (
    <div style={{
      ...
      gap: 8, padding: '8px 0', borderBottom: '1px solid #f1f5f9', fontSize: 13,
    }}>
...
export default function LabOrderPage() {
  ...
  /* ---- Modal yêu cầu làm lại (requestRetest) ---- */
  /* Backend tự copy medicalRecord/doctor/labTechnician/priority/notes từ order cũ.
     Frontend chỉ cần gửi rejectionReason để backend gán vào previousOrder. */
  ...
  /* ---------- Fetch — dùng getLabOrderQueue, backend filter theo role ---------- */
  const fetchOrders = useCallback(async () => {
    setLoading(true)
    try {
      // GET /api/v1/lab/queue — backend tự nhận ra role DOCTOR và trả orders của bác sĩ này
  ...
  /* ---------- Approve: PUT /api/v1/lab/{id}/approve ---------- */
  ...
  /* ---------- Request retest: PUT /api/v1/lab/{id}/retest ---------- */
  /*  Backend tự lấy medicalRecord / doctor / labTechnician / priority / notes
   *  từ previousOrder — frontend chỉ cần gửi rejectionReason.              */
  ...
  return (
    <>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid #f1f5f9', backgroundColor: '#f8fafc' }}>
                    {['STT', 'Ngày tạo', 'Bệnh nhân', 'SĐT', 'Dịch vụ XN', 'Ưu tiên', 'Trạng thái', ''].map((h) => (
      ...
      {/* ================================================================ */}
      {/*  Modal: Xem chi tiết kết quả & Duyệt / Yêu cầu làm lại        */}
      {/* ================================================================ */}
      <Modal
        open={reviewModal}
      ...
      >
        {selectedOrder && (
          <div>
                ...
                <div style={{ marginBottom: 16 }}>
                  ...
                  {/* {resultDetail.imageUrl && (
                    <div style={{ marginTop: 16 }}>
                      <div style={{ fontSize: 12, color: '#94a3b8', marginBottom: 6 }}>Ảnh kết quả xét nghiệm</div>
                      <img
                        src={resultDetail.imageUrl} alt="Lab Result"
            ...
            {/* Action buttons — chỉ hiện khi SUBMITTED */}
            {selectedOrder.status === 'SUBMITTED' && (
              <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end', paddingTop: 16, borderTop: '1px solid #f1f5f9' }}>
                ...
                <Button
                  type="primary" loading={approving} onClick={handleApprove}
      ...
      {/* ================================================================ */}
      {/*  Modal: Yêu cầu làm lại xét nghiệm (requestRetest)             */}
      {/*  Gọi PUT /api/v1/lab/{id}/retest với LabOrderRequest            */}
      <Modal
        open={retestModal}
      ...
      >
        <div style={{ padding: '8px 0' }}>
          {/* Tóm tắt thông tin order đang từ chối */}
          {selectedOrder && (
            <div style={{
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
            ...
            <Button
              danger type="primary" loading={retesting} onClick={handleRetest}
              style={{ fontSize: 13 }}
            >
